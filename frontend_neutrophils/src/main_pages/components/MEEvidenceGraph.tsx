...
export default class MEEvidenceGraph extends React.Component<MEEvidenceGraphProps, MEEvidenceGraphState> {
    ...
    render() {
        ...
        for (let i = 0; i < this.state.newData.length; ++i)
        {
            ...
            var messengerCount = dataElem['messengers_info'].length;
            var categoriesCount = dataElem['categories_info'].length;

            graphData.links.push({source: sourceIdx, target: targetIdx, evidence: messengerCount, predicted: categoriesCount})
