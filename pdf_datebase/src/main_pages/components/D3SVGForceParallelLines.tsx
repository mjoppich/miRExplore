...
export default class D3SVGParallelLinesGraph extends React.Component<D3SVGParallelLinesProps, D3SVGParallelLinesState> {
    ...
    drawChart(theprops) {
        ...
        this.force = d3.forceSimulation<SimNode, SimLink>() 
            ...
            .force("repel", d3.forceManyBody().strength(-15).distanceMax(250).distanceMin(100))
            .force("collide", d3.forceCollide().radius(function(d)
            {
                return 75;
            }).iterations(8).strength(0.85));
/*
        ...
        var color = function (group) {
            if (group == 'Human Genes') {
                return "#aaaaaa"
            } else if (group == 'miRNAs') {
                return "#fbc280"
            } else if (group == "Gene Ontology") {
                return "#405275" 
            } else {
                return "#80c2fb"
            }
    ...
    group1Link
    .style("stroke-width", function stroke(d)  {return 0; })
    .style("stroke", "#70C05A")
